...
namespace SortingAlgosVar
{
            ...
            while (cont)
            {
                ...
                switch (input)
                {
                    ...
                    case SortingAlgos.algos.B:
                        ...
                        break;
                    case SortingAlgos.algos.I:
                        Algorithm.InsertionSortMin(arr);
                        Algorithm.printArray(arr, arr.Length);
                        break;
                    case SortingAlgos.algos.S:
    ...
    class SortingAlgos
    {
        ...
        public void InsertionSortMin(int[] arr)
        {
            int n = arr.Length;
            for (int i = n-2; i >= 0; i--)
            {
        ...
        public void QuickSortRand(int[] arr, int left, int right)
        {
            if (left < right)
            {
                int PartIn = QuickSortRandPart(arr, left, right);
                QuickSortRand(arr, left, PartIn - 1);
                QuickSortRand(arr, PartIn + 1, right);
            }
        }
        private int QuickSortRandPart(int[] arr, int left, int right)
        {
            Random rand = new Random();
            int i = rand.Next(left, right);
            int pivot = arr[i];
            arr[i] = arr[right];
            arr[right] = pivot;

            return QuickSortRandPartition(arr, left, right);
        }

        private int QuickSortRandPartition(int[] arr, int left, int right)
        {
            int pivot = arr[right];
            int temp;
            int i = left;
